...
export function generateAnalysisPDF(result: AnalysisResult, jobTitle: string): Buffer {
  ...
  sections.forEach(([name, section]) => {
    ...
    doc.fontSize(11).font('Helvetica-Bold').fillColor(color).text(`${name}: ${section.score}/100 — ${section.label}`);
